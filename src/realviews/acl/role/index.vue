...
//请求方法
import { reqRemoveRole, reqAllRoleList, reqAddOrUpdateRole, reqAllMenuList, reqSetPermisstion } from '@/views/acl/role';
import type { RoleResponseData, Records, RoleData, MenuResponseData, MenuList } from '@/views/acl/role/type'
//引入骨架的仓库
...
const setPermisstion = async (row: RoleData) => {
    ...
    if (result.code == 200) {
        ...
        selectArr.value = filterSelectArr(menuArr.value, []);//也就是说filterSelectArr函数 会返回initArr值的数组 来做勾选
    }
}
//树形控件的测试数据
const defaultProps = {
...
// 递归 ; 因为要过滤的是四级的id (因为呃...一两句话解释不清 看课 p119 就知道为什么要过滤四级了 因为一级的化 一勾选 那么下面的所有都会被勾选上 就没意义了)
const filterSelectArr = (allData: any, initArr: any) => {
    allData.forEach((item: any) => {
        if (item.select && item.level == 4) {//4级 为 true的 push进来
            initArr.push(item.id);
        }
        if (item.children && item.children.length > 0) {//符合条件的 进入递归
...
const handler = async () => {
    ...
    //选中节点的ID getCheckedKeys是elementplus tree组件自带的方法 getHalfCheckedKeys也是
