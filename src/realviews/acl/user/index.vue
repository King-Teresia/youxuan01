...
import useLayOutSettingStore from '@/store/modules/layout.config.ts'
import { ref, onMounted, reactive, nextTick } from 'vue';
import { reqSelectUser, reqRemoveUser, reqUserInfo, reqAddOrUpdateUser, reqAllRole, reqSetUserRole } from '@/views/acl/user'
import type { SetRoleData, UserResponseData, Records, User, AllRoleResponseData, AllRole } from '@/views/acl/user/type';
// @ts-ignore
...
const validatorPassword = (rule: any, value: any, callBack: any) => {
    ...
    if (/^[a-zA-Z]\w{5,17}$/.test(value)) {
        ...
        callBack(new Error('长度在6~18之间,只能包含字母、数字和下划线'))
    }
}
//表单校验的规则对象
const rules = {
...
const handleCheckedCitiesChange = (value: string[]) => {
    console.log(value); //是当前勾选的选项值 (数组)
    //顶部复选框的勾选数据
