    ...
    <el-card style="margin:10px 0px">
        ...
        <div v-show="scene == 0">
            ...
            <el-table border style="margin:10px 0px" :data="attrArr">
                ...
                <el-table-column label="属性名称" width="120px" align="center" prop="attrName">
                </el-table-column>
                <el-table-column label="属性值名称">
                    <!-- row:即为当前属性值对象 -->
                    <template #="{ row }">
                        <!-- prop相当于一层的row.  (对于:data绑定/收集的收据来说)-->
...
//引入获取已有属性与属性值接口
import { reqAttr, reqAddOrUpdateAttr, reqRemoveAttr } from '@/views/product/attr';
import type { AttrResponseData, Attr, AttrValue } from '@/views/product/attr/type';
//获取分类的仓库
...
const toLook = (row: AttrValue, $index: number) => {
    ...
    if (row.valueName.trim() == '') {
        //删除调用对应属性值为空的元素(因为你不能将空的传给服务器所以要删除splice 删一个)
    ...
    let repeat = attrParams.attrValueList.find((item) => {
        //切记把当前失却焦点的属性值对象(row) 从当前数组扣除判断 具体操作如下
        if (item != row) { // 把当前的row 从数组中 扣出去 然后再在 数组中 查找 相同的valueName 并返回
...
const toEdit = (row: AttrValue, $index: number) => {
    ...
    row.flag = true;
    //nextTick:响应式数据发生变化后,获取更新的DOM(组件实例)
    nextTick(() => {
